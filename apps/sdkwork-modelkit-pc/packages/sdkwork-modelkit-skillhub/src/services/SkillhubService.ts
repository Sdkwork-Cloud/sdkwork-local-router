...
let mockSkills: SkillItem[] = [
  ...
  { 
    id: 2, name: 'Skill Vetter', author: '@spclaudehome', downloads: '237k', rating: 1.1, type: 'Security', updated: '1d ago',
    desc: 'Security-first skill vetting for AI agents. Use before installing any skill from CloudHub, GitHub, or other sources. Checks for red flags, permission scope, and suspicious patterns.', 
  ...
  { 
    ...
    desc: 'Typed knowledge graph for structured agent memory and composable skills. Use when creating/querying entities (Person, Project, Task, Event, Document), linkin...', 
...
export class LocalSkillhubService implements ISkillhubService {
  ...
  async installSkillToAgents(skillId: number, agents: string[]): Promise<void> {
    const item = mockSkills.find(s => s.id === skillId);
    if(item) {
...
export const skillhubService = new LocalSkillhubService();
