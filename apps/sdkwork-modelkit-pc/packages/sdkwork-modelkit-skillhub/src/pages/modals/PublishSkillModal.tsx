...
export function PublishSkillModal({ isOpen, onClose, categories, onSkillPublished }: PublishSkillModalProps) {
  ...
  const [newSkillCategory, setNewSkillCategory] = useState('Workflows');
  const [newSkillDesc, setNewSkillDesc] = useState('');
  const [newSkillPermissions, setNewSkillPermissions] = useState('');
  const [newSkillSchemaType, setNewSkillSchemaType] = useState('REST API');
  const [newSkillAuthType, setNewSkillAuthType] = useState('None');
  ...
  const [skillZipFileName, setSkillZipFileName] = useState('');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newSkillName || !newSkillAuthor || !newSkillDesc) {
      toast.error('Please fill in Name, Author, and Description');
      return;
    }
    if (publishSourceType === 'repo' && !skillRepoUrl) {
      toast.error('Please fill in the Repository URL');
      return;
    }
    if (publishSourceType === 'zip' && !skillZipFileName) {
    ...
    try {
      const newSkill = await skillhubService.publishSkill({
        name: newSkillName,
        author: newSkillAuthor.startsWith('@') ? newSkillAuthor : `@${newSkillAuthor}`,
        category: newSkillCategory,
        desc: newSkillDesc,
        schemaType: newSkillSchemaType || 'REST API',
        authType: newSkillAuthType || 'None',
        permissions: newSkillPermissions ? newSkillPermissions.split(',').map(p => p.trim()) : ['Memory Access'],
        sourceType: publishSourceType,
        repoUrl: publishSourceType === 'repo' ? skillRepoUrl : undefined,
        fileName: publishSourceType === 'zip' ? skillZipFileName : undefined
      });
      ...
      <div className="bg-panel border border-divider rounded-2xl w-full max-w-lg shadow-2xl flex flex-col max-h-[85vh] animate-in fade-in zoom-in duration-200">
        <div className="px-5 py-4 border-b border-divider flex justify-between items-center bg-canvas rounded-t-2xl shrink-0">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-bold text-text-main">Publish Dynamic Tool/Skill</h2>
          </div>
          ...
          <form id="publishForm" onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-text-main">Skill/Tool Name <span className="text-red-500">*</span></label>
                <input 
                  type="text" 
                  value={newSkillName}
            ...
            {publishSourceType === 'repo' && (
              <div className="space-y-1.5 animate-in slide-in-from-top-2 duration-300">
                ...
                <p className="text-[10px] text-text-muted px-1">Ensure the repository contains a valid manifest (skill.json)</p>
              </div>
            )}
            ...
            {publishSourceType === 'zip' && (
              <div className="space-y-1.5 animate-in slide-in-from-top-2 duration-300">
                <label className="text-xs font-bold text-text-main flex items-center gap-1.5">
                  ...
                  Compressed Skill Artifact <span className="text-red-500">*</span>
                </label>
                <div 
                ...
                >
                  ...
                  {skillZipFileName ? (
                    <div className="flex flex-col items-center">
                      ...
                      <span className="text-[13px] font-bold text-emerald-500">{skillZipFileName}</span>
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-text-main">Skill Category Taxonomy <span>*</span></label>
              <select 
            ...
            <div className="space-y-1.5">
              ...
              <textarea 
                ...
                placeholder="What capabilities does this skill grant to AI models?"
                rows={3}
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                ...
                <select 
                  value={newSkillSchemaType}
                  onChange={(e) => setNewSkillSchemaType(e.target.value)}
              ...
              <div className="space-y-1.5">
                ...
                <select 
                  value={newSkillAuthType}
                  onChange={(e) => setNewSkillAuthType(e.target.value)}
                  className="w-full bg-surface border border-divider rounded-lg px-3 py-2.5 text-[13px] text-text-main focus:border-emerald-500 transition-all custom-select"
                >
            ...
            <div className="space-y-1.5">
              ...
              <input 
                type="text" 
                value={newSkillPermissions}
                onChange={(e) => setNewSkillPermissions(e.target.value)}
