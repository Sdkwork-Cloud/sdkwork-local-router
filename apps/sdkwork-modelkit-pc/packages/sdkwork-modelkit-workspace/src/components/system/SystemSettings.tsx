...
export function SystemSettings({ onNavigate }: SystemSettingsProps) {
  ...
  const handleClearCache = async () => {
    ...
    toast.success(
      t('workspace:cache_flushed', "System local router and component download cache flushed successfully! Cache flushed (~48.2 MB saved).")
    );
  };
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-canvas overflow-y-auto custom-scrollbar">
      ...
      <div className="px-8 pb-12 grid grid-cols-1 xl:grid-cols-2 gap-6 w-full max-w-none">
        ...
        {/* Port Ingress and Registry module */}
        <div className="space-y-6">
          {/* Active port status view */}
          <div className="border border-divider-strong bg-panel rounded-2xl p-6 shadow-xl space-y-4">
            <h3 className="text-sm font-bold text-text-main flex items-center gap-2 border-b border-divider pb-3">
              ...
              {t('workspace:local_router_nic_status', 'Local Router NIC Status')}
            </h3>

            <div className="space-y-3">
              <div className="flex items-center justify-between p-3 rounded-xl bg-surface border border-divider hover:border-divider-strong transition-colors">
                <div className="flex items-center gap-2.5">
                  <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
                  <div>
                    <span className="text-xs font-bold text-text-main block">
                      {t('workspace:workspace_ingress_tls', 'Workspace Ingress TLS')}
                    </span>
                    <span className="text-[10px] text-text-muted">
                      127.0.0.1:3000
                    </span>
                ...
                <span className="text-[10px] font-mono bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 px-2 py-0.5 rounded">
                  {t('workspace:ok_host_enabler', 'OK (Host Enabler)')}
                </span>
              </div>
              ...
              <div className="flex items-center justify-between p-3 rounded-xl bg-surface border border-divider hover:border-divider-strong transition-colors">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <span className="text-xs font-bold text-text-main block">
                      {t('workspace:local_api_routing_gateway', 'Local Api Routing Gateway')}
                    </span>
                    <span className="text-[10px] text-text-muted">
                      LOCALHOST:11434
                    </span>
              ...
              <div className="flex items-center justify-between p-3 rounded-xl bg-surface border border-divider hover:border-divider-strong transition-colors">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <span className="text-xs font-bold text-text-main block">
                      {t('workspace:claude_engine_bridge', 'Claude Engine Bridge (IPC)')}
                    </span>
                    <span className="text-[10px] text-text-muted">
                      LOCALHOST:11435
                    </span>
                ...
                <span className="text-[10px] font-mono bg-amber-500/15 border border-primary-main/25 text-primary-main px-2 py-0.5 rounded">
                  {t('workspace:standby', 'STANDBY')}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="border border-divider-strong bg-panel rounded-2xl p-6 shadow-xl space-y-4">
            <h3 className="text-sm font-bold text-text-main flex items-center gap-2 border-b border-divider pb-3">
              ...
              {t("workspace:txt_1325")}
            </h3>

            <div className="flex items-center justify-between">
              <div>
                <span className="text-xs font-bold text-text-main block">
                  {t("workspace:txt_1326")}
                </span>
                <span className="text-[10px] text-text-muted block mt-0.5">
                  {t("workspace:txt_1327")}
                  <strong className="text-text-muted font-mono">48.2 MB</strong>
                </span>
              </div>
              <button
                type="button"
