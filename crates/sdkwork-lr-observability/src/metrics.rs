...
use prometheus::{
    self, Encoder, IntCounter, IntGauge, HistogramOpts, HistogramVec, Opts, Registry, TextEncoder,
};
...
pub static REQUEST_DURATION: LazyLock<HistogramVec> = LazyLock::new(|| {
    HistogramVec::new(
        HistogramOpts::new("sdkwork_request_duration_seconds", "Request duration in seconds")
...
pub fn init_metrics() {
    ...
    REGISTRY.register(Box::new(REQUESTS_FAILURE.clone())).unwrap();
    REGISTRY.register(Box::new(REQUEST_DURATION.clone())).unwrap();
    REGISTRY.register(Box::new(ACTIVE_ACCOUNTS.clone())).unwrap();
    REGISTRY.register(Box::new(DEGRADED_ACCOUNTS.clone())).unwrap();
    REGISTRY.register(Box::new(CIRCUIT_OPEN_ACCOUNTS.clone())).unwrap();
    REGISTRY.register(Box::new(CIRCUIT_OPENS_TOTAL.clone())).unwrap();
